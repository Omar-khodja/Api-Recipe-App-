    ...
    return Scaffold(
      ...
      body: favoriteMeals.when(
        data: (meals) => meals.isEmpty
            ?  Center(child: Text("Favorite meals is empty ",style: Theme.of(context).textTheme.titleLarge,))
            : ListView.builder(
                itemCount: meals.length,
                itemBuilder: (context, index) {
                  final meal = meals[index];
                  return MealsCard(meal: meal, openMealDeatailsScreen: () {openMealDeatailsScreen(context, meal);});
                },
              ),
